...
#### one mentor match to 1~3 mentees
#### max matches = the number of mentees. So some mentors have no match
...
# Duplicating each mentor node three times to allow up to three matches
for mentor in match_scores_df['Mentor'].unique():
    for i in range(1, 4):  # Duplicate each mentor 3 times
        B.add_node(f"mentor_{mentor}_{i}", bipartite=0)
        
# Add nodes for mentees without duplication
for mentee in match_scores_df['Mentee'].unique():
    B.add_node(f"mentee_{mentee}", bipartite=1)
    
# Add edges between mentors and mentees
for idx, row in match_scores_df.iterrows():
    mentor = row['Mentor']
    mentee = row['Mentee']
    match_score = row['Match Score']
    # Connect each mentor node duplicate to each mentee
    for i in range(1, 4):
        B.add_edge(f"mentor_{mentor}_{i}", f"mentee_{mentee}", weight=-match_score)
...
# Clean up the mentor identifiers by removing the duplication index
for index, row in final_matches_df.iterrows():
    # Extract the original mentor email before the first underscore (removes the _i suffix)
    original_mentor = re.sub(r"_\d+$", "", row['Mentor'])  # Remove the numerical suffix
    final_matches_df.at[index, 'Mentor'] = original_mentor

# Now sort the DataFrame by 'Match Score' descending, then 'Mentor' and 'Mentee' ascending
...
# Output the results
print('Exporting match_2.2_matches.csv')
print(final_matches_df.head())
final_matches_df.to_csv('match_2.2_matches.csv', index=False)
