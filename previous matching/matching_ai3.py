...
additional_rewards = [3, 2, 1.5]  # 1st, 2nd, and 3rd respectively
...
for mentor_email, mentor_row in mentors_df.iterrows():
    ...
    for mentee_email, mentee_row in mentees_df.iterrows():
        ...
        # Check each attribute for overlap and importance
        for attr in ['Roles', 'Industry', 'Company Stage', 'Topics', 'In-Person Meeting Location']:
            mentor_attr_values = set(mentor_row[attr].split('; '))
            mentee_attr_values = set(mentee_row[attr].split('; '))

            # Check for overlap
            if mentor_attr_values & mentee_attr_values:  # Intersection (overlap) of sets
                attr_match_score += base_overlap_score

                # Check the ranking in 'Most Important Attributes' and add additional rewards
                for rank, reward in enumerate(additional_rewards, start=1):
                    if attr in mentor_important_attrs[:3]:  # Check if the attr is in the top 3 for the mentor
                        mentor_rank = mentor_important_attrs.index(attr) + 1
                        if mentor_rank == rank:
                            attr_match_score += reward
                    if attr in mentee_important_attrs[:3]:  # Check if the attr is in the top 3 for the mentee
                        mentee_rank = mentee_important_attrs.index(attr) + 1
                        if mentee_rank == rank:
                            attr_match_score += reward
        match_score += additional_rewards
        ...
        match_scores.append({
            ...
            'YOE Score': yoe_score,
            'Attribute Match Score': attr_match_score,
