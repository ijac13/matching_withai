...
# mentees = mentees.groupby(level=0, axis=1).apply(lambda x: x.apply(same_merge, axis=1)) - original
mentees = mentees.T.groupby(level=0).apply(lambda x: x.apply(same_merge)).T ##AI
...
#     # print("matched_mentors",matched_mentors)
#     # print("matched_mentees",matched_mentees)
...
# print("matched_list_result", matched_list_result) #check
...
## AI 
def match_pairs_v2(combined_df, max_mentees_per_mentor):
    # Initialize dictionaries to track the number of matches for each mentor and mentee
    mentor_matches_count = {email: 0 for email in combined_df['Email-mentor'].unique()}
    mentee_matches_count = {email: 0 for email in combined_df['Email-mentee'].unique()}

    # Initialize an empty list to store final matched pairs
    final_matched_pairs = []

    # Iterate over each row in the combined DataFrame
    for index, row in combined_df.iterrows():
        mentor_email = row['Email-mentor']
        mentee_email = row['Email-mentee']

        # Check if the mentor and mentee have not exceeded their max allowed matches
        if mentor_matches_count[mentor_email] < max_mentees_per_mentor and mentee_matches_count[mentee_email] < 1:
            # Check YOE restriction: Mentor's YOE should be greater than Mentee's YOE
            if float(row['Avg Year of YOE-mentor']) > float(row['Avg Year of YOE-mentee']):
                # Update the match counts for both mentor and mentee
                mentor_matches_count[mentor_email] += 1
                mentee_matches_count[mentee_email] += 1

                # Add the current match to the final list of matched pairs
    ...
    matched_pairs_df = pd.DataFrame(final_matched_pairs)

    return matched_pairs_df

# Call the rewritten matching function with the combined DataFrame and the max mentees per mentor
matched_results_df = match_pairs_v2(combined, max_mentees_per_mentor)

# Save the final matched pairs to a CSV file
matched_results_df.to_csv('matched_results_v2.csv', index=False)

results_wide = mentors_filtered.join(mentees_filtered.set_index('Email'), on='Email', rsuffix='-mentor', lsuffix='-mentee')
results_wide.to_csv('matched_v2.csv', index=False, sep='\t')
results_wide.to_csv('matched_wide_v2.csv', index=False, sep='\t')
