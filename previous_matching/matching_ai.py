...
def calculate_overlap_and_score(mentor_attrs, mentee_attrs, mentor_importance, mentee_importance, importance_weights, base_overlap_score):
    ...
    # debug_mentor_email = '97agupta@gmail.com'
    # debug_mentee_email = 'alekprus97@gmail.com'
...
# Initialize missing columns only
...
final_columns_order =  ['Mentor', 'Mentee', 'Offset Difference', 'Offset Score', 
                        'YOE Difference', 'YOE Score', 'Total Overlaps', 
                        'Overlap Score', 'Open Answer Score', 'Applied to Mentor Score', 'Match Score']
match_scores_df = match_scores_df[final_columns_order]
# Export csv 
...
#4.1 Sort the DataFrame by 'Match Score' in descending order to prioritize higher scores
sorted_matches = match_scores_df.sort_values(by=['Mentor', 'Match Score','YOE Score','Offset Score','Counts of Overlap',
...
# Count unique mentors who got matched
unique_mentors_matched = sorted_matches['Mentor'].nunique()
unique_mentees_matched = sorted_matches['Mentee'].nunique()
...
#4.2 Initialize empty DataFrames to keep track of matched mentors and mentees
matched_mentors = pd.DataFrame(columns=sorted_matches.columns)
matched_mentees = set()

#4.3 Iterate over the sorted matches
...
#4.4 Reset index for the matched_mentors DataFrame
...
# Count unique mentors who got matched
unique_mentors_matched = matched_top2['Mentor'].nunique()
unique_mentees_matched = matched_top2['Mentee'].nunique()

print(f"Number of mentors who got matched for match_6: {unique_mentors_matched}")
print(f"Number of mentees who got matched for match_6: {unique_mentees_matched}")


#4.5 Leave only email and Match score
...
#5.1 Statistical Analysis of Match Scores
# - check matchcurrent_analysis.py
# - previous one, check matchprevious_analysis.py

#5.2 Count unique mentors who got matched
unique_mentors_matched = match_scores_df['Mentor'].nunique()
unique_mentees_matched = match_scores_df['Mentee'].nunique()

print(f"Number of mentors who got matched for match_7: {unique_mentors_matched}")
print(f"Number of mentees who got matched for match_7: {unique_mentees_matched}")
